...
export default function LastRace({navigation}){
        ...
        Axios.get("https://ergast.com/api/f1/current/last/results").then(
            (response)=>{ 
              ...
              setRaceData({
                  ...
                  RaceDateMonth:Time.getFullYear() + "/" + Time.getMonth()+ "/"+Time.getDate()
              })
    ...
    return(
        <Card>
                ...
                <Text style={{fontFamily:"TitilliumWeb_300Light",fontSize:21}}>{RaceData.RaceDate}</Text>
                <Text style={{fontFamily:"TitilliumWeb_300Light",fontSize:13}}>{RaceData.RaceTime}</Text>
