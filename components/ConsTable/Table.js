...
export default function ConsTable(props){
  ...
  <View style={!props.topThree ? {flex:1} : {}}>
    ...
    <FlatList 
        ...
        <ListItem key={item[2]} onPress={()=>{!props.topThree ? CarStore.update(s=>{s.index = item[2]}) : {}}} containerStyle={[(selectedKey==item[2] && !props.topThree ) ? {backgroundColor:"#F30000"}:{} ]} bottomDivider>
          <Text style={[(selectedKey==item[2] && !props.topThree ) ?  {color:"#eee"}:{},{fontFamily:'TitilliumWeb_400Regular'}]}>{item[0]}</Text>
          <ListItem.Content>
            <ListItem.Title style={[(selectedKey==item[2] && !props.topThree ) ? {color:"#eee"}:{},{fontFamily:"TitilliumWeb_700Bold"} ]}>
              {item[2]} 
            </ListItem.Title>
            <ListItem.Subtitle style={[(selectedKey==item[2] && !props.topThree ) ? {color:"#eee"}:{},{fontFamily:'TitilliumWeb_300Light'}]}>{item[3]}</ListItem.Subtitle>
            </ListItem.Content>
          <ListItem.Content right>
              <Text style={[(selectedKey==item[2] && !props.topThree ) ?  {color:"#eee"}:{},{fontFamily:'TitilliumWeb_400Regular'}]}>{item[1]} PTS</Text>
          </ListItem.Content>
        </ListItem>
        )}
